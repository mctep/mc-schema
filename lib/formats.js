...
function formats() {
	this.addFormat('date-time', function(type, value) {
		if (type === 'string') {
			...
			if (!__dt_d || !__dt_t) {
				this.addError({ code: 'FORMAT_ERROR' });
				__dt_err = true;
			}
	...
	this.addFormat('date', function(type, value) {
		if (type === 'string') {
			if (!value) {
				this.addError({ code: 'FORMAT_ERROR' });
			} else {
	...
	this.addFormat('uri', function(type, value) {
		if (type === 'string') {
			if (!value) {
				this.addError({ code: 'FORMAT_ERROR' });
			} else {
	...
	this.addFormat('email', function(type, value) {
		if (type === 'string') {
	...
	this.addFormat('ipv4', function(type, value) {
		if (type === 'string') {
			if (value.indexOf('.') === -1) {
				this.addError({ code: 'FORMAT_ERROR' });
			} else {
	...
	this.addFormat('ipv6', function(type, value) {
		if (type === 'string') {
	...
	this.addFormat('hostname', function(type, value) {
		if (type === 'string') {
			if (value.length > 255) {
				this.addError({ code: 'FORMAT_ERROR' });
			} else {
