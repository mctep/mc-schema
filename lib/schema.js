...
Schema.prototype.jsonPath = jsonPath;
...
function Schema(fn, schema, env, root) {
	...
	if (schema.$ref) {
		if (schema.$ref[0] === '#' && (!schema.$ref[1] || schema.$ref[1] === '/')) {
			schema.__$ref = this.getSchemaByPath(schema.$ref);
		} else {
			schema.__$ref = this.env.compiled[schema.$ref];
		}
	}
}

function validate(data, opts) {
	var result = this.__validate(data, opts && opts.collectErrors);
...
function getSchemaByPath(path) {
	...
	if (!schema.__schema && this.rootSchema.schema !== schema) {
		this.env.compile(schema, this.rootSchema);
	}
...
function coerce(data, iteration) {
	...
	this.validate(data, { collectErrors: true });
