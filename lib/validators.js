...
function validators() {
	...
	this.addValidator('items', function(value, type, schema, self) {
		if (type === 'array') {
			...
			var __i_is_array = Array.isArray(schema.__items);

			if (__i_is_array && value.length > schema.__items.length) {
				this.addError({
					message: 'ARRAY_ITEMS_BIG_LENGTH',
				...
				while (__i_idx--) {
					...
					if (__i_is_array) {
						__i_schema = schema.__items[__i_idx];
					} else {
